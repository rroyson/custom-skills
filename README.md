...
| `/suno-lyric-cowrite` | Co-writer on your own draft: grills you for the feeling, story, and themes when they aren't settled, one question with options at a time; then names the stall, offers three directions, and candidate lines in your diction. Ask it to deslop a draft and it returns a before/after table for every line it would change | `lyrics/<slug>/lyrics.md`: Themes as they settle, only the lines you accept |
...
- `scripts/check_lyrics.py` runs on every song: blocks tier-1 words, scores tier-2 density, reads rhyme schemes and syllable spread, flags lazy rhyme pairs, chorus drift, a section that ends on the chorus's title word just before the chorus, tag mistakes, digits, and bad Style or Exclude text. `--self-test` verifies it.
...
- Each song folder holds `lyrics.md`, the workspace (Themes, then Lyrics with bare section tags), and `song.md`, the paste-ready Suno prompts built from it. Word changes go into lyrics.md first. A whole new draft moves the old Lyrics section to `old-drafts.md`; Themes you built stay unless the request contradicts them. `suno-lyric-cowrite/themes.md` is the theme-tension list the grill offers when you don't know what a song is about yet.
- `lyrics/voice.md` is per project and optional. Everything in it is a guideline, never a lock: defaults, dated preferences, observed habits from lyrics you wrote, influence trait cards, samples, hated words. The request always wins. Taste notes the writing skills hear park in `lyrics/voice-proposed.md` right away; say yes in the moment or run `/suno-voice review` later.
...
- Both writing skills re-read craft.md, words.txt, and voice.md on every invocation. Started partway through a conversation, they used to skip the files and drift from the rules (2026-09-11).
- Adopted 2026-09-11 from bitwize-music-studio/claude-ai-music-skills (CC0): the over-explained image, pep-talk phrase, and triumph-on-schedule tells (voice-checker), the pre-chorus echo check (lyric-reviewer), the before/after pass table (lyric-refiner), literal vocal texture words (tips-and-tricks), emo and dance-punk genre notes. Left out: the MCP server, album and research pipelines, asking about every homograph, and `model:` pins, since a skill's `model` covers only its first turn and follow-ups run on the session model.
